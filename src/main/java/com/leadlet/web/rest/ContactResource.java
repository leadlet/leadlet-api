...
public class ContactResource {
    ...
    @GetMapping("/contacts")
    @Timed
    public ResponseEntity<List<ContactDTO>> getContacts(@ApiParam String q, @ApiParam Pageable pageable) {
    ...
    /**
     * DELETE  /contacts/:idList : delete the "idList" contact.
     *
     * @param idList the id of the contactDTO to delete
     * @return the ResponseEntity with status 200 (OK)
     */
    @DeleteMapping("/contacts/{idList}")
    @Timed
    public ResponseEntity<List<Long>> deleteContact(@PathVariable List<Long> idList) {
        log.debug("REST request to delete Contact : {}", idList);
