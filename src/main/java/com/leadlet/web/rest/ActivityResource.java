...
public class ActivityResource {
     ...
     * @return the ResponseEntity with status 200 (OK) and the list of activities in body
     */
    @GetMapping("/activities/organization/{id}")
    @Timed
    public ResponseEntity<List<ActivityDTO>> getActivitiesByOrganizationId(@ApiParam Pageable pageable, @PathVariable Long id) {
        log.debug("REST request to get a page of Activities");
        Page<ActivityDTO> page = activityService.findByOrganizationId(id, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/activities");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
