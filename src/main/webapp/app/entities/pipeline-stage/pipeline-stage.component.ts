...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/pipeline-stage'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/pipeline-stage', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: PipelineStage) {
        return item.id;
    }
    registerChangeInPipelineStages() {
        this.eventSubscriber = this.eventManager.subscribe('pipelineStageListModification', (response) => this.loadAll());
    }
