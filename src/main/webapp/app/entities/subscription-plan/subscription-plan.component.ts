...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/subscription-plan'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: SubscriptionPlan) {
        return item.id;
    }
    registerChangeInSubscriptionPlans() {
        this.eventSubscriber = this.eventManager.subscribe('subscriptionPlanListModification', (response) => this.loadAll());
    }
